...
data = pd.read_csv('./r-55.09(1).csv', header=None)  # 读取数据，此行需修改
...
def regre(x, z, z_data=data_z_mean):  # 修正坐标为x、z
    ...
    for xi in trange(len(X)):
        for zi in range(len(Z)):
            for ri in range(len(R)):
                x_hg, z_hg = regre(X[xi], Z[zi])  # x_hg和z_rg为修正后的样品表面坐标
...
if __name__ == '__main__':
    ...
    xzr = find_xzr(x_cir, z_cir, r_cir)
    change_cir(xzr, 50)
    print('\n参数搜索中，搜索步长50nm')
    xzr50 = find_xzr(x_cir, z_cir, r_cir)
    change_cir(xzr50, 20)
    print('\n参数搜索中，搜索步长20nm')
    xzr20 = find_xzr(x_cir, z_cir, r_cir)
    change_cir(xzr20, 10)
    print('\n参数搜索中，搜索步长10nm')
    xzr10 = find_xzr(x_cir, z_cir, r_cir)
    change_cir(xzr10, 5)
    print('\n参数搜索中，搜索步长5nm')
    xzr5 = find_xzr(x_cir, z_cir, r_cir)
    change_cir(xzr5, 1)
    print('\n参数搜索中，搜索步长1nm')
    xzr1 = find_xzr(x_cir, z_cir, r_cir)
    ra = round(slow_cal(xzr1), 2)
    print('\n表面粗糙度为：{}'.format(ra))
